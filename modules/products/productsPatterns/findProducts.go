...
func (b *findProductBuilder) openJsonQuert() {
	b.query += `
		SELECT
...
func (b *findProductBuilder) whereQuery() {
	...
	// Search check
	if b.req.Search != "" {
		b.values = append(b.values, b.req.Search)
		queryWhereStack = append(queryWhereStack, `
...
func (b *findProductBuilder) sort() {
	...
	if sortMap[b.req.SortBy] != "" {
		b.req.SortBy = sortMap["ASC"]
	} else {
		b.req.SortBy = sortMap[strings.ToUpper(b.req.SortBy)]
	}
...
func (b *findProductBuilder) Count() int {
	...
	if err := b.db.Get(&count, b.query, b.values...); err != nil {
		log.Printf("Error count product: %v", err)
		return 0
	}
